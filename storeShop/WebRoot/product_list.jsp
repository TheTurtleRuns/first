				...
				<c:forEach items="${pageBean.rows }" var="product" varStatus="vs">
					<c:if test="${vs.count==1 || vs.count==7 }">
						...
						<div class="col-md-2" style="text-align: center;">
							<a href="${pageContext.request.contextPath }/ProductServlet?method=findProductInfoByPid&pid=${product.pid}">
								<img src="${pageContext.request.contextPath}/${product.pimage}" width="170" height="170" style="display: inline-block;">
							</a>
							<p><a href="${pageContext.request.contextPath }/ProductServlet?method=findProductInfoByPid&pid=${product.pid}" style='color:green'>${product.pname }</a></p>
							<p><font color="#FF0000">商城价：&yen;${product.shop_price }</font></p>
						</div>
					<c:if test="${vs.count==6 || vs.count==12 || pageBean.size==vs.count }">
						</div>
					</c:if>
				</c:forEach>
			</c:if>
			...
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${not empty pageBean }">
					<c:choose>
						...
						<c:otherwise>
							<li >
								<a href="${pageContext.request.contextPath}/ProductServlet?method=findAllProductByCidForPage&pageNumber=${param.pageNumber-1}&cid=${param.cid}" aria-label="Previous">
									<span aria-hidden="true">&laquo;</span>
								</a>
							</li>
						</c:otherwise>
				...
				<c:if test="${not empty pageBean }">
					<c:forEach begin="1" end="${pageBean.totalPage }" var="i">
						<c:choose>
							...
							<c:otherwise>
								<li><a href="${pageContext.request.contextPath}/ProductServlet?method=findAllProductByCidForPage&pageNumber=${i}&cid=${param.cid}">${i }</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</c:if>
				...
				<c:if test="${not empty pageBean }">
					<c:choose>
						...
						<c:otherwise>
							<li>
								<a href="${pageContext.request.contextPath}/ProductServlet?method=findAllProductByCidForPage&pageNumber=${param.pageNumber+1}&cid=${param.cid}" aria-label="Next">
									<span aria-hidden="true">&raquo;</span>
								</a>
							</li>
						</c:otherwise>
